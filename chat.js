...
exports.handler = async function(event, context) {
  ...
  try {
    ...
    const { prompt, lang = 'zh', context, accessToken } = requestBody;
    
    // 验证访问令牌
    if (accessToken !== process.env.ACCESS_TOKEN) {
      console.log('Unauthorized: invalid or missing accessToken');
      return {
        statusCode: 401,
        headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
        body: JSON.stringify({ error: 'Unauthorized', message: 'Invalid access token' })
      };
    }
